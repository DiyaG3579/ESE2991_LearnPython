...
def euclidean2(x, y, coordinates2):
    distance2 = 100
    for coordinate2 in coordinates2:
        point1 = num.array([x, y])
        point2 = num.array(coordinate2)

        distance = num.linalg.norm(point1-point2)
        
        if distance < distance2:
            distance2 = distance
            current2 = coordinate2
...
while ((abs(xi - xt) > 0.25) or (abs(yi - yt) > 0.25)) and i <= 2000:
    ...
    for path in septagon_paths:
        if path.contains_point((xi, yi), radius = 0.1):
            inside = True
    if inside == False:
        pointc = euclidean2(xi, yi, coordinates2)
        pointb = [xi, yi]
        pointa = steer(pointc, pointb, epsilon)
        for path in septagon_paths:
            if path.contains_point(pointa, radius = 0.1):
        ...
        if not line_hits_obstacle(pointb, pointc, septagon_paths) and insideout == False:
            xi = pointa[0]
            yi = pointa[1]
            ax.plot(xi, yi, '.', color = 'magenta')
            ax.plot([pointa[0], pointc[0]],[pointa[1], pointc[1]],'--',color='magenta')
            #ax.plot(xi, yi, '.', color = 'black')
            coordinates2.append([xi, yi])
        else:
            insideout = False
...
ax.set_aspect('equal')
ax.set_title("Two Car Enviornment Test")
